...
class Solution {
  public:
    void InOrder(Node* root, vector<pair<int,Node*>> &in, vector<int> &ch)
    {
        if(!root) return;
        InOrder(root->left,in,ch);
        in.push_back({root->data,root});
        ch.push_back(root->data);
        InOrder(root->right,in,ch);
        return;
    }
    
    struct Node *correctBST(struct Node *root) {
        // code here
        vector<pair<int,Node*>> in;
        vector<int> ch;
        InOrder(root, in, ch);
        ...
        for(int i=0; i<in.size(); i++)
        {
            if(in[i].first != ch[i])
            {
                if(temp == NULL) temp = in[i].second;
                else{
                    int x = temp->data;
                    temp->data = in[i].first;
                    in[i].second->data = x;
                    // return root;
                    break;
                }
